...
def hold_out_items_fast(data, cols, indptr, holdout, seed):
    ...
    for row in range(num_rows):
        ...
        tr_data[tr_idx: tr_idx + split_idx] = data[start_idx: end_idx][non_zero_perm[:split_idx]]
        tr_col[tr_idx: tr_idx + split_idx] = cols[start_idx: end_idx][non_zero_perm[:split_idx]]
        tr_indptr[row] = tr_idx
        te_data[te_idx: te_idx + (non_zero_count - split_idx)] = data[start_idx: end_idx][non_zero_perm[split_idx:]]
        te_col[te_idx: te_idx + (non_zero_count - split_idx)] = cols[start_idx: end_idx][non_zero_perm[split_idx:]]
    ...
    return (tr_data[: tr_indptr[-1]], tr_col[: tr_indptr[-1]], tr_indptr),\
           (te_data[: te_indptr[-1]], te_col[: te_indptr[-1]], te_indptr)

class CSRDataset():
    def __init__(self, filename, user_holdout=0.10, int_holdout=0.15, r_seed=98765, binarize=False, toy=False):
        self.random_state = np.random.RandomState(seed=r_seed)
        
        if toy:
            self.data = self.data = sparse.load_npz("{}".format(filename)).astype(np.int32)[:, :10000]
        ...
        self.strong_val_tr, self.strong_val_te = self.hold_out_items(self.data[rand_idx[train_idx:]], self.int_holdout, r_seed)

        # Indices to split data into train/val/test for weak generalization
        self.weak, self.weak_test = self.hold_out_items(self.data, self.int_holdout, r_seed)
        self.weak_train, self.weak_val = self.hold_out_items(self.weak, self.int_holdout, r_seed)
    ...
    def hold_out_items(self, uim, holdout, r_seed):
        ...
        :return:
        """
        tr_csr, te_csr = hold_out_items_fast(uim.data, uim.indices, uim.indptr, holdout, r_seed)

        return sparse.csr_matrix(tr_csr, shape=uim.shape), sparse.csr_matrix(te_csr, shape=uim.shape)
...
class MultVAE(nn.Module):
    def __init__(self, model_conf, dataset, device, lr=1e-3):
        ...
        if isinstance(model_conf['enc_dims'], str):
            model_conf.enc_dims = eval(model_conf['enc_dims'])  # Some kind of function to evaluate

        self.enc_dims = [self.num_items] + model_conf['enc_dims']  # Dimensions of encoding [in,
        ...
        self.anneal = 0.  # Current annealing value
        ...
        self.logdir = '/scratch/kyle709/study_2/vae_checkpoints/{}_{}_{}_{}'.format(self.enc_dims, self.dropout, self.anneal_cap, self.total_anneal_steps)
        ...
        for i, (d_in, d_out) in enumerate(zip(self.enc_dims[:-1], self.enc_dims[1:])):
            if i == len(self.enc_dims[:-1]) - 1:  # If encoded layer double output for mean and dev.
                d_out *= 2
            self.encoder.append(nn.Linear(d_in, d_out))  # Add a linear layer with appropriate in/out
            if i != len(self.enc_dims[:-1]) - 1:  # If not encoding layer add act. function
                self.encoder.append(nn.Tanh())

        # This block builds the decoder by appending modules to a list iteratively
        self.decoder = nn.ModuleList()
        for i, (d_in, d_out) in enumerate(zip(self.dec_dims[:-1], self.dec_dims[1:])):
            self.decoder.append(nn.Linear(d_in, d_out))  # Add a linear layer with appropriate in/out
            if i != len(self.dec_dims[:-1]) - 1:  # If not encoding layer add act. function
    ...
    def forward(self, uim, novel=True):
        ...
        # Encoder
        h = F.dropout(F.normalize(uim), p=self.dropout, training=self.training)  # Apply dropout (0 out half values)
        ...
        epsilon = torch.zeros_like(std_q).normal_(mean=0, std=0.01)  # Some more fancy stats

        samples = mu_q + self.training * epsilon * std_q  # Compute samples from the distribution
    ...
    def train_one_epoch(self, batch_size, val=True, verbose=False):
        ...
        num_batches = int(np.ceil(num_training / batch_size))  # Count of batches to complete epoch
            ...
            for b in range(num_batches):
                ...
                if (b + 1) * batch_size >= num_training:
                    batch_matrix = torch.FloatTensor(
                        pred[b * batch_size:].toarray()).to(self.device)

                    batch_holdout = holdout[b * batch_size:]
                else:
                    batch_matrix = torch.FloatTensor(
                        pred[b * batch_size: (b + 1) * batch_size].toarray()).to(self.device)
    ...
    def train_val(self, epochs, batch_size, tr_batch_size, patience=float('inf'), start=1):
        ndcg_list = []
        early_stop = 0
        anneal_patience = 5
        for i in tqdm(range(start, epochs + 1)):
            ...
            self.eval()
            ndcg_dist = self.predict_eval(self.dataset.weak_train, self.dataset.weak_val, tr_batch_size)
